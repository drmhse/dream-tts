...
fn main() -> Result<()> {
    ...
    for (label, c, len) in [("stage0 256ch@8040", 256usize, 8040usize), ("stage1 128ch@48240", 128, 48240)] {
        ...
        for k in [3usize, 7, 11] {
            ...
            let mut a = || { tts_nn::centered_conv1d_gemm(&x, &w_tap, None, k, 1, (k - 1) / 2).unwrap(); Ok(()) };
            let mut b = || { cols(&x, k, 1)?; Ok(()) };
            let mut cc = || { w_tap.matmul(&cols_k)?; Ok(()) };
            let mut dd = || { tts_nn::tapconv::centered_conv1d_fused_with(&x, &w_tap, None, k, 1, (k - 1) / 2, "conv1d_tap_gemm_f32").unwrap(); Ok(()) };
            let mut ee = || { tts_nn::tapconv::centered_conv1d_fused(&x, &w_tap, None, k, 1, (k - 1) / 2).unwrap(); Ok(()) };
        ...
        let mut variants: Vec<(&str, &mut dyn FnMut() -> candle_core::Result<()>)> = Vec::new();
        let mut e1 = || { x.mean_keepdim(2)?; Ok(()) };
        let mut e2 = || { tts_nn::fused::sub_sqr(&x, &mean)?.mean_keepdim(2)?; Ok(()) };
        let mut e3 = || { tts_nn::fused::adain_apply(&x, &mean, &var, &gamma, &beta, 1e-5)?; Ok(()) };
        let mut e4 = || { tts_nn::fused::snake_beta(&x, &alpha, &recip)?; Ok(()) };
        let mut e5 = || { (&x + &x)?; Ok(()) };
        ...
        let mut e6 = || { tts_nn::fused::moments(&x)?; Ok(()) };
        let mut e7 = || { tts_nn::fused::adain_snake(&x, &mean.flatten_all()?, &var.flatten_all()?, &gamma, &beta, &alpha.flatten_all()?, &recip.flatten_all()?, 1e-5)?; Ok(()) };
        ...
        let mut variants: Vec<(&str, &mut dyn FnMut() -> candle_core::Result<()>)> = Vec::new();
        let mut g1 = || { w1.matmul(&cols_k)?; Ok(()) };
        let mut g2 = || { w3.matmul(&cols_k)?; Ok(()) };
        let mut g3 = || { w1h.matmul(&cols_h)?; Ok(()) };
        let mut g4 = || { w3h.matmul(&cols_h)?; Ok(()) };
